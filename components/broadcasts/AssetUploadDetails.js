...
function AssetUploadDetails({ selectedStream }) {
  console.log(' AssetUploadDetails.js:6 ', selectedStream)
  ...
  return (
    <Card style={{ color: 'black', padding: '0px' }}>
      {selectedStream.downloadUrl ? (
        <div>
          <Player
            playbackId={`${selectedStream.downloadUrl}`}
      ...
      <Card.Body>
        ...
        <p>Recording:</p>
        {selectedStream.record ? 'Recording stream' : 'Not recording stream'}
      </Card.Body>
    </Card>
  )
}

export default AssetUploadDetails
